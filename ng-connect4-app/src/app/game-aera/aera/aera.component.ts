      ...
      <div *ngIf="this.GameService.isWinner" id="myModal" class="modal" myModal>
        ...
        <div class="modal-content">
          ...
          <p class="modal-content-message">
            {{ currentRound.nextPlayerTurn.name }} win !
          </p>
          <hr class="separator" />
          <div class="modal-content-btn">
            <button class="btn" type="button" (click)="this.reload()">
              again ?
            </button>
            <button class="btn" type="button" (click)="this.toGoHome()">
              new Challenger
            </button>
          </div>
        </div>
...
export class AeraComponent implements OnInit {
  ...
  public joueur2: Players = {
    name: this.Login.players![1],
    color: Color.Yellow,
  };
